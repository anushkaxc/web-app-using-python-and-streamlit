...
data_url = '/Users/anushkac/Desktop/py and st/Motor_Vehicle_Collisions_-_Crashes.csv'

st.title("Motor Vehicle Collisions in NYC")
st.markdown("Streamalit Dashboard for analysing vehicle collisions in NYC 💥")


@st.cache(persist=True) #not rerun everytime it is loaded, only when input is change
def load_data(nrows):
...
injured_people=st.slider("Max number of persons injured in collisions", 0, 19)
st.map(data.query("injured_persons>= @injured_people")[['latitude', 'longitude']].dropna(how='any'))


st.header("How many collisions at a given time of day?")
...
layers=[
    pdk.Layer("HexagonLayer", data= data[['date/time', 'latitude', 'longitude']],
